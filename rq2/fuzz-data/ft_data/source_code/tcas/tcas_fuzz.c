...
char *argv[];
{
    ...
    if (argc < 2)
    {
        fprintf(stderr, "Error: No filepath specified!\n");
        exit(1);
    }
    ...
    for (int i = 0, j = 0; j < 23; i++, j+=2) {
        // Convert pair of bytes into a 16-bit integer (big-endian)
        inputs[i] = (buffer[j] << 8) | buffer[j+1];
    }
    // -------------------------------------------------------------------------
    ...
    Cur_Vertical_Sep = inputs[0];
    High_Confidence = inputs[1];
    Two_of_Three_Reports_Valid = inputs[2];
    Own_Tracked_Alt = inputs[3];
    Own_Tracked_Alt_Rate = inputs[4];
    Other_Tracked_Alt = inputs[5];
    Alt_Layer_Value = inputs[6];
    Up_Separation = inputs[7];
    Down_Separation = inputs[8];
    Other_RAC = inputs[9];
    Other_Capability = inputs[10];
    Climb_Inhibit = inputs[11];
