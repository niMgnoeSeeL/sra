...
def frequency_estimation(obss: np.ndarray) -> np.ndarray:
    if np.sum(max(row) for row in obss) == 0:
        return np.zeros(obss.shape[1])
    return np.sum(obss, axis=0) / np.sum(max(row) for row in obss)


def laplace_estimation(obss: np.ndarray, alpha: float) -> np.ndarray:
    return (np.sum(obss, axis=0) + alpha) / (
        np.sum(max(row) for row in obss) + 2 * alpha
    )
...
def structure_estimation_path_specific(
    ...
    """
    # TODO: implement real path-specific structure estimation
    return (1.0, 0, 0)
...
def structure_estimation(
    ...
    if target_node in covered_nodes:
        return (
            node2numcovered[target_node] / np.sum(max(row) for row in obss),
            0,
            -1,
        )
        ...
        if source_node in covered_nodes:
            ...
            if len(graph.edge_dict[extended_source]) == 2:
                ...
                if child1 in covered_nodes or child2 in covered_nodes:
                    ...
                    counterfactual_prob = (
                        (
                            node2numcovered[source_node]
                            / np.sum(max(row) for row in obss)
                        )
                        * alpha
                    ...
                    if debug:
                        print(
                            f"{source_node=}, num_covered={node2numcovered[source_node]}, counterfactual_prob={(node2numcovered[source_node] / np.sum(max(row) for row in obss))} * {alpha / (node2numcovered[source_node] + 2 * alpha)} = {counterfactual_prob}"
                        )
