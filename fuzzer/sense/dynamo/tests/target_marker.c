...
size_t __dr_end_tracking_sink(uint32_t sink_id) {
  // Empty stub - DynamoRIO wraps this
  // Returns: max distance read from ptr offset
  return 0;
}

// This is our "taint sink" function - now wrapped with markers
// Sink ID 42: SSL_write-like function
...
void *worker_thread(void *arg) {
  ...
  for (int iter = 0; iter < NUM_ITERATIONS; iter++) {
    ...
    // Note: len removed - DR does dynamic allocation lookup
    __dr_start_tracking_sink(sink_id, (void *)buf);
    taint_sink(buf, len);
    // End tracking - returns max distance read from buf
    size_t max_read = __dr_end_tracking_sink(sink_id);
    
    if (max_read > 0) {
      fprintf(stderr, "[Thread %d iter %d] Max read distance: %zu bytes\n",
