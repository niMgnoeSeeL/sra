...
event_app_instruction_inline_guard(void *drcontext, void *tag, instrlist_t *bb,
  ...
  /* Register plan:
   *   - RDX: TLS per-thread data pointer; reused to hold each computed EA.
   *   - RCX: active flag (ECX) for JECXZ; also used only for the guard.
  ...
  for (int i = 0; i < num_srcs; ++i) {
    ...
    /* Compute EA into RDX (reg_tdata reused as EA holder).
     * drutil needs a separate scratch register: we use R8.
     */
    drutil_insert_get_mem_addr(drcontext, bb, instr, src, reg_tdata,
                               reg_scratch);
    ...
    dr_insert_clean_call_ex(
        drcontext, bb, instr, (void *)on_mem_read, false /* no fp save */, 2,
        opnd_create_reg(reg_tdata), OPND_CREATE_INT32(mem_size));
  }
